...
$errors = [];
$old = ['name' => '', 'email' => '', 'phone' => '', 'shelter_name' => '', 'reg_no' => '', 'capacity' => '', 'address' => '', 'city' => '', 'country' => ''];
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $old = compact('name', 'email', 'phone', 'shelter_name', 'reg_no', 'capacity', 'address', 'city', 'country');
  ...
  if (!$shelter_name) $errors[] = "Please enter shelter name.";
  if (!$reg_no) $errors[] = "Please enter registration number.";
  if ($capacity <= 0) $errors[] = "Please enter valid capacity.";
  if (!$address) $errors[] = "Please enter your shelter address.";
  ...
  // File validation
  $logoImg = $_FILES['logo_image'] ?? null;
  $docFile = $_FILES['reg_doc'] ?? null;
  if (!$logoImg || $logoImg['error'] !== UPLOAD_ERR_OK) $errors[] = "Please upload shelter logo.";
  if (!$docFile || $docFile['error'] !== UPLOAD_ERR_OK) $errors[] = "Please upload registration document.";
  ...
  if (!$errors) {
    ...
    $role = 'shelter';
    $status = 'pending'; // shelters also need admin approval
    ...
    // Upload files
    $logoPath = "uploads/shelters/logo_" . $user_id . ".jpg";
    $docPath = "uploads/shelters/doc_" . $user_id . ".pdf";
    move_uploaded_file($logoImg['tmp_name'], __DIR__ . "/" . $logoPath);
    move_uploaded_file($docFile['tmp_name'], __DIR__ . "/" . $docPath);

    // Insert into shelters
    $stmt2 = $conn->prepare("INSERT INTO shelters(user_id, shelter_name, reg_no, capacity, address, city, country, logo_image, reg_doc) VALUES(?,?,?,?,?,?,?,?,?)");
    $stmt2->bind_param("ississsss", $user_id, $shelter_name, $reg_no, $capacity, $address, $city, $country, $logoPath, $docPath);
    $stmt2->execute();
  ...
  <div class="col-md-9 col-lg-8">
    <div class="card card-soft p-4 p-md-5">
      <div class="d-flex align-items-center mb-3">
        <span class="logo-badge me-2"><i class="bi bi-house-heart text-white"></i></span>
        <h1 class="h4 mb-0 brand-text">Register as Shelter</h1>
      </div>
      ...
      <form method="post" enctype="multipart/form-data" novalidate>
        ...
        <div class="mb-3">
          ...
          <input name="shelter_name" class="form-control" required value="<?php echo htmlspecialchars($old['shelter_name']); ?>">
        </div>
        <div class="mb-3">
          <label class="form-label">Registration Number</label>
          <input name="reg_no" class="form-control" required value="<?php echo htmlspecialchars($old['reg_no']); ?>">
        </div>
        <div class="mb-3">
          <label class="form-label">Capacity (Number of pets)</label>
          <input type="number" name="capacity" class="form-control" required value="<?php echo htmlspecialchars($old['capacity']); ?>">
        </div>
        <div class="mb-3">
          <label class="form-label">Address</label>
          <input name="address" class="form-control" required value="<?php echo htmlspecialchars($old['address']); ?>">
        </div>
        ...
        <div class="mb-3">
          <label class="form-label">Registration Document (PDF)</label>
          <input type="file" name="reg_doc" class="form-control" accept="application/pdf" required>
        </div>
        ...
        <button class="btn btn-primary w-100">Register as Shelter</button>
      </form>
    </div>
  </div>
</div>
