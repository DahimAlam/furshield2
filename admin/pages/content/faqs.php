...
if($_SERVER['REQUEST_METHOD']==='POST' && hash_equals($csrf,$_POST['csrf']??'')){
  ...
  if($action==='add'){
    ...
    $a=trim($_POST['answer']??'');
    $s=(int)($_POST['is_active']??1);
    $st=$conn->prepare("INSERT INTO faqs (question,answer,is_active) VALUES (?,?,?)");
    $st->bind_param("ssi",$q,$a,$s);$st->execute();$st->close();
    $msg='FAQ added.';
  }
  if($action==='edit'){
    $id=(int)$_POST['id'];
    ...
    $a=trim($_POST['answer']??'');
    $s=(int)($_POST['is_active']??1);
    $st=$conn->prepare("UPDATE faqs SET question=?,answer=?,is_active=? WHERE id=?");
    $st->bind_param("ssii",$q,$a,$s,$id);$st->execute();$st->close();
    $msg='FAQ updated.';
  }
  if($action==='delete'){
    $id=(int)$_POST['id'];
    $st=$conn->prepare("DELETE FROM faqs WHERE id=?");
...
$total=(int)($conn->query("SELECT COUNT(*) c FROM faqs")->fetch_assoc()['c']??0);
$res=$conn->prepare("SELECT * FROM faqs ORDER BY created_at DESC LIMIT ?,?");
...
<main class="main p-4">
  <div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h1 class="h3 fw-bold">Manage FAQs</h1>
      <button class="btn text-white" style="background:var(--primary);border-radius:14px" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-plus-lg"></i> New FAQ</button>
    </div>
    ...
    <div class="card border-0" style="border-radius:var(--radius);box-shadow:var(--shadow);background:var(--card)">
      <div class="table-responsive">
        <table class="table align-middle mb-0">
          <thead class="table-light"><tr><th>ID</th><th>Question</th><th>Status</th><th>Created</th><th></th></tr></thead>
          <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                <form method="post" class="d-inline">
                  ...
                  <button class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete this FAQ?')">Delete</button>
                </form>
              </td>
            </tr>
                ...
                <div class="modal-content rounded-4">
                  <form method="post">
                    ...
                    <div class="modal-body">
                      <div class="mb-3"><label class="form-label">Question</label><input type="text" name="question" class="form-control" value="<?php echo e($r['question']);?>"></div>
                      <div class="mb-3"><label class="form-label">Answer</label><textarea name="answer" class="form-control" rows="5"><?php echo e($r['answer']);?></textarea></div>
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content rounded-4">
      <form method="post">
        ...
        <div class="modal-header"><h5 class="modal-title">New FAQ</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
          <div class="mb-3"><label class="form-label">Question</label><input type="text" name="question" class="form-control"></div>
          <div class="mb-3"><label class="form-label">Answer</label><textarea name="answer" class="form-control" rows="5"></textarea></div>
        ...
        <div class="modal-footer"><button class="btn text-white" style="background:var(--primary)">Add FAQ</button></div>
      </form>
    </div>
  </div>
</div>
