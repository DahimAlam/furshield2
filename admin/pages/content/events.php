...
$uploadsDir=$root.'/uploads/events/';
...
if($_SERVER['REQUEST_METHOD']==='POST' && hash_equals($csrf,$_POST['csrf']??'')){
  ...
  if(in_array($action,['add','edit'])){
    ...
    if(!empty($_FILES['image']['name']) && is_uploaded_file($_FILES['image']['tmp_name'])){
      ...
      if(in_array($ext,['jpg','jpeg','png','webp','gif'])){
        $fname='e-'.bin2hex(random_bytes(4)).'.'.$ext;
        if(move_uploaded_file($_FILES['image']['tmp_name'],$uploadsDir.$fname)){
    ...
    if($action==='add'){
      $st=$conn->prepare("INSERT INTO events (title,date,city,link,image,is_active) VALUES (?,?,?,?,?,?)");
      $st->bind_param("sssssi",$title,$date,$city,$link,$imgPath,$status);
      $st->execute();$st->close();
      $msg='Event added.';
    }else{
      $id=(int)$_POST['id'];
      $sql="UPDATE events SET title=?,date=?,city=?,link=?,is_active=?".($imgPath?",image=?":"")." WHERE id=?";
      $st=$conn->prepare($sql);
      if($imgPath){
        // 7 params
        $st->bind_param("ssssssi",$title,$date,$city,$link,$status,$imgPath,$id);
      }else{
        // 6 params
        $st->bind_param("sssssi",$title,$date,$city,$link,$status,$id);
      }
      $st->execute();$st->close();
      $msg='Event updated.';
    }
  ...
  if($action==='delete'){
    $id=(int)$_POST['id'];
    $st=$conn->prepare("DELETE FROM events WHERE id=?");
...
$total=(int)($conn->query("SELECT COUNT(*) c FROM events")->fetch_assoc()['c']??0);
$res=$conn->prepare("SELECT * FROM events ORDER BY date DESC LIMIT ?,?");
...
<main class="main p-4">
  <div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h1 class="h3 fw-bold">Manage Events</h1>
      <button class="btn text-white" style="background:var(--primary);border-radius:14px" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-plus-lg"></i> New Event</button>
    </div>
    ...
    <div class="card border-0" style="border-radius:var(--radius);box-shadow:var(--shadow);background:var(--card)">
      <div class="table-responsive">
        <table class="table align-middle mb-0">
          <thead class="table-light"><tr><th>ID</th><th>Image</th><th>Title</th><th>Date</th><th>City</th><th>Status</th><th></th></tr></thead>
          <tbody>
          <?php foreach($rows as $r): ?>
            <tr>
              <td><?php echo (int)$r['id'];?></td>
              <td><?php if(!empty($r['image'])): ?><img src="<?php echo e(BASE.$r['image']);?>" style="height:50px;width:80px;object-fit:cover;border-radius:6px"><?php endif;?></td>
              ...
              <td>
                ...
                <form method="post" class="d-inline">
                  ...
                  <button class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete this event?')">Delete</button>
                </form>
              </td>
            </tr>
                ...
                <div class="modal-content rounded-4">
                  <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                      ...
                      <div class="mb-3"><label class="form-label">Date</label><input type="date" name="date" class="form-control" value="<?php echo e($r['date']);?>"></div>
                      <div class="mb-3"><label class="form-label">City</label><input type="text" name="city" class="form-control" value="<?php echo e($r['city']);?>"></div>
                      <div class="mb-3"><label class="form-label">Link</label><input type="url" name="link" class="form-control" value="<?php echo e($r['link']);?>"></div>
    ...
    <div class="modal-content rounded-4">
      <form method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3"><label class="form-label">Link</label><input type="url" name="link" class="form-control"></div>
          <div class="mb-3"><label class="form-label">Image</label><input type="file" name="image" class="form-control"></div>
        ...
        <div class="modal-footer"><button class="btn text-white" style="background:var(--primary)">Add Event</button></div>
      </form>
    </div>
  </div>
</div>
